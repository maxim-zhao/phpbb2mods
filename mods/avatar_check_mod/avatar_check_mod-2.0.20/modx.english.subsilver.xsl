...
/* $Id: modx.english.subsilver.xsl,v 1.1 2006/02/23 15:05:56 david Exp $ */
...
<xsl:if test="@type = 'regex'">
									...
									<xsl:if test="name() = 'inline-action'">
										<xsl:if test="@type = 'after-add'">
											<h3>In-line Add after</h3>
										</xsl:if>
										<xsl:if test="@type = 'before-add'">
											<h3>In-line Add before</h3>
										</xsl:if>
										<xsl:if test="@type = 'replace-with'">
											<h3>In-line Replace With</h3>
										</xsl:if>
										<xsl:if test="@type = 'operation'">
											<h3>In-line Increment</h3>
	...
	<xsl:template name="give-filez">
		<dl>
			<xsl:for-each select="mod:file">
				<dt>Copy: <xsl:value-of select="mod:from" /></dt>
				<dd>To: <xsl:value-of select="mod:to" /></dd>
			</xsl:for-each>
		</dl>
	</xsl:template>
