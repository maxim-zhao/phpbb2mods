...
{
	$body = ereg_replace('MEMBERNAMEHERE', stripslashes(htmlspecialchars($userdata['username'])), stripslashes($newsletter_htmlemail));
}
...
if(strpos($body,"<title>") === false)
{
	...
	{
		$body = "<title>" . $archive_subject . "</title>\n" . $body;
	}
	else
	{
		$body = str_replace("<head>", "<head>\n<title>" . $archive_subject . "</title>\n", $body);
	}
}
