...
$sql = "SELECT user_id, portal_links FROM " . $table_prefix . "users
	WHERE portal_links <> ''
	ORDER BY user_id";
if( !$result = $db->sql_query ($sql) )
{
...
else
{
	...
	{
		$portal_links = $row['portal_links'];
		$user = $row['user_id'];
		$user_portal_links = explode(',', $portal_links);

		$j = 0;
		for ( $i = 0; $i < count($user_portal_links); $i++)
		{
			if ( $user_portal_links[$i] != '' )
			{
				$sql2 = "SELECT portal_link FROM " . $table_prefix . "user_portal_links
					 WHERE user_id = $user
					 AND portal_link = ".$user_portal_links[$i];
				...
				else
				{
					...
					{
						$sql3 = "INSERT INTO " . $table_prefix . "user_portal_links
							 (user_id, portal_link, portal_sort) 
							 VALUES ($user, ".$user_portal_links[$i].", $j)";
		...
		$sql4 = "SELECT portal_link FROM " . $table_prefix . "user_portal_links
			 WHERE user_id = $user
		...
		else
		{
			...
			{
				$portal_correct_sort = $row4['portal_link'];
				$j += 10;
				$sql5 = "UPDATE " . $table_prefix . "user_portal_links
					 SET portal_sort = $j
					 WHERE user_id = $user
...
$sql = array();
$sql[] = "ALTER TABLE " . $table_prefix . "users DROP portal_links";
$sql[] = "ALTER TABLE " . $table_prefix . "users DROP portal_sort";
