...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			$template->set_filenames(array(
				"body" => "modcp/user_avatar_gallery.tpl")
			);
	...
	else
	{
		...
		$template->set_filenames(array(
			"body" => "modcp/user_edit_body.tpl")
		);

		if ( ( $this_userdata['user_level'] == ADMIN ) || ( ( $this_userdata['user_level'] == MOD ) && ( $userdata['user_id'] != $this_userdata['user_id'] ) ) )
		{
			$template->assign_block_vars('switch_change_disallowed', array());
		}
		else
		{
			$template->assign_block_vars('switch_change_allowed', array());
		}
		...
		$template->assign_vars(array(
			'DISABLE_CHANGE' => ( ( $this_userdata['user_level'] == ADMIN ) || ( ( $this_userdata['user_level'] == MOD )&& ( $userdata['user_id'] != $this_userdata['user_id'] ) ) ) ? ' disabled="disabled"' : '',
			'USERNAME' => $username,
			'EMAIL' => $email,
			'YIM' => $yim,
			'ICQ' => $icq,
...
else
{
	...
	//
	$template->set_filenames(array(
		'body' => 'modcp/user_select_body.tpl')
	);
