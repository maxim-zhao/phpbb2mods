 ...
 * File Name:       admin_spamwords_config.php
 * File Path:       admin/
...
$admins = $db->sql_fetchrowset($result2);
$select_box = '<select name="pm_sender">';
for ($i = 0; $i < count($admins); $i++)
{
	$selected = ($admins[$i]['user_id'] == $new['pm_sender']) ? ' selected="selected"' : '';
	$select_box .= '<option value="' . $admins[$i]['user_id'] . '"' . $selected . '>' . $admins[$i]['username'] . '</option>';
}
$select_box .= '</select>';
...
$template->set_filenames(array("body" => "admin/spamwords_config_body.tpl"));
...
(
	...
	'L_PM_SELECT_ADMIN' => $lang['PM_select_admin'],
...
{
	$activated_words .= ($i == 0) ? $words[$i]['spam_word'] : ', ' . $words[$i]['spam_word'];
}

$template->assign_vars(array(
	'WORDS' => $activated_words,
	'L_NO_WORDS' => $no_words
));
