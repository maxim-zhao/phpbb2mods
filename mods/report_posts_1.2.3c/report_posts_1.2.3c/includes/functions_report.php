...
{
	global $db, $board_config, $template, $lang, $phpbb_root_path, $phpEx, $userdata;
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		$template->assign_block_vars('postrow', array(
			...
			'REPORTER'			=> '<a href="' . $phpbb_root_path . 'profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['reporter_id'] . '">' . $row['username'] . '</a>',
			'COMMENTS'			=> $comments,
			...
			'U_CLOSE_REPORT'	=> ( $row['report_status'] == REPORT_POST_NEW ) ? append_sid($phpbb_root_path . 'viewpost_reports.' . $phpEx . '?mode=closereport&amp;report=' . $row['report_id']) : append_sid($phpbb_root_path . 'viewpost_reports.' . $phpEx . '?mode=openreport&amp;report=' . $row['report_id']))
		);
...
function create_comments($row)
{
	global $db, $board_config, $lang, $phpbb_root_path, $phpEx;
		...
		if ( $row['last_action_user_id'] != 0 && $row['last_action_time'] != 0 )
		{
			...
			$last_action_user = '<a href="' . $phpbb_root_path . 'profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['last_action_user_id'] . '">' . $row2['username'] . '</a>';
