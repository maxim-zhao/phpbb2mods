...
if ( ($mode == 'closereport' || $mode == 'openreport') && $report_id != '' )
{
	...
	if ( empty($last_action_comments) )
	{
		...
		$template->assign_vars(array(
			...
			'S_ACTION'		=> append_sid($phpbb_root_path . 'viewpost_reports.'.$phpEx.'?mode='.$mode.'&amp;report='.$report_id))
		);
	...
	else
	{
		...
		$last_action_user = '<a href="' . $phpbb_root_path . 'profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $userdata['user_id'] . '">' . $userdata['username'] . '</a>' ;
		...
		$last_action_comments = '<b>' . $last_action . '</b>' . "\n" . $last_action_comments;
...
else if ( $mode == 'close' || $mode == 'open' )
{
	...
	$mass_action_comments = '<b>' . $last_action . '</b>' . "\n" . $lang['Last_action_checkbox'];
	...
	if ( empty($reports) )
	{
		$message =  $lang['Report_not_selected'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid($phpbb_root_path . 'viewpost_reports.'.$phpEx) . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
