 ...
 *   *** The SQL/DDL statements contained in this script are optimized for PostgreSQL only!
 *
 * - Installation:
...
$sql[] = 'CREATE TABLE ' . $table_prefix . 'actions (
    ...
    CONSTRAINT ' . $table_prefix . 'actions_pk PRIMARY KEY (action_id)
)';
$sql[] = 'CREATE TABLE ' . $table_prefix . 'actions_performed (
    ...
    CONSTRAINT ' . $table_prefix . 'actions_performed_pk PRIMARY KEY (id)
)';
$sql[] = 'CREATE UNIQUE INDEX ' . $table_prefix . 'performed_id ON ' . $table_prefix . 'actions_performed (action_id, performer_id, post_id)';
$sql_count = count($sql);
