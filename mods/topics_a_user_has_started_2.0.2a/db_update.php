...
//
$sql = array();
$sql[] = 'SELECT COUNT(*) AS `user_topics`, `topic_poster` AS `user_id` FROM `' . TOPICS_TABLE . '` GROUP BY `topic_poster`';
$sql_count = count($sql);
...
while ($row = $db->sql_fetchrow($result))
{
	$sql = "UPDATE " . USERS_TABLE . "
		SET user_topics = " . $row['user_topics'] . "
	...
	if (!$db->sql_query($sql))
	{
		...
		$success = $lang['Error'] . ':';
		$class = ($i%2) == 0 ? 'row1' : 'row2';
		$sql_rows .= '<tr><td class="'.$class.'"><div class="genmed">' . $sql . ';<br /><br /><b style="color:' . $color . ';">' . $success . '</b>' . $errmsg . '</div></td></tr>';
	}
}
