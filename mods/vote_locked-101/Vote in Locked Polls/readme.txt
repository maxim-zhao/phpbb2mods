VOTE IN LOCKED POLLS
A phpBB extension by Thoul
Current Release: 1.0.1
Official Supported phpBB Release: 2.0.21
...
In normal phpBB operations, users cannot do anything in a locked topic.
Replies, editing, poll voting, and all the other various options are not
allowed.  This modification will remove the restrictions that locking a topic
imposes on poll voting.  While the rest of the topic remains off limits, the 
poll will behave as if the topic is not locked.

This feature will be useful for polls where you may not wish for members to be
able to post comments, without having a separate forum with a special permission
set up for polls only.

--------------------------------------------------------------------------------
                                    FEATURES
--------------------------------------------------------------------------------

-Allows members to vote in polls of locked topics.
-Includes alternate install instructions for Categories Hierarchy.
...
Version 1.0.1 - October 17, 2006
A potential parse error in viewtopic.php has been corrected and a Replace
instruction for the same file has been clarified.
