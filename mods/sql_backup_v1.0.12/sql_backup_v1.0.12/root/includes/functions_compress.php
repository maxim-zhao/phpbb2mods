...
*
* @package phpBB3
* @version $Id: functions_compress.php 7 2006-04-13 18:05:54Z vic $
...
class compress_zip extends compress
{
	...
	function download($filename, $attachment_filename)
	{
		global $phpbb_root_path;
		...
		header("Content-Type: $mimetype; name=\"$attachment_filename.zip\"");
		header("Content-Disposition: attachment; filename=\"$attachment_filename.zip\"");
		header('Content-Length: ' . filesize("{$phpbb_root_path}$filename.zip"));

		$fp = fopen("{$phpbb_root_path}$filename.zip", 'rb');
...
class compress_tar extends compress
{
	...
	function download($filename, $attachment_filename)
	{
		...
		header('Pragma: no-cache');
		header("Content-Type: $mimetype; name=\"$attachment_filename.$this->type\"");
		header("Content-Disposition: attachment; filename=\"$attachment_filename.$this->type\"");
		header('Content-Length: ' . filesize("{$phpbb_root_path}$filename.$this->type\""));

		$fp = fopen("{$phpbb_root_path}$filename.$this->type", 'rb');
		while ($buffer = fread($fp, 1024))
		{
			echo $buffer;
		}
