############################################################## 
## MOD Title: Last post info - upgrade from 1.0.1
...
#-----[ FIND ]----- 
#
			$last_post_info = sprintf($lang['last_post_info'], '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row[0]['user_id']) . '">', $row[0]['username'], '</a>', '<a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . "=" . $row[0]['post_id'] . '#' . $row[0]['post_id']) . '">', create_date($board_config['default_dateformat'], $row[0]['post_time'], $board_config['board_timezone']), '</a>');
# 
#-----[ REPLACE WITH ]----- 
# 
...
#-----[ FIND ]----- 
# 
			$last_post_info = sprintf($lang['last_post_info'], '', $row[0]['username'], '', '<a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . "=" . $row[0]['post_id'] . '#' . $row[0]['post_id']) . '">', create_date($board_config['default_dateformat'], $row[0]['post_time'], $board_config['board_timezone']), '</a>');
# 
#-----[ REPLACE WITH ]----- 
# 
...
#-----[ FIND ]----- 
# 
$lang['last_post_info'] = 'Last post by <b>%s%s%s</b> on %s%s%s'; // a href, usernname, /a, a href, post_time, /a
# 
#-----[ REPLACE WITH ]----- 
# 
