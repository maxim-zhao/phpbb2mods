...
$profilelink = "profile.php?mode=editprofile&";
...
if (( isset($HTTP_POST_VARS['submit']) ) || (isset($HTTP_POST_VARS['submitavatar'])))
{
	...
	if ( !empty($new_password) && !empty($password_confirm) )
	{
		...
		else
		{
				...
				if (( $row['user_password'] != md5($cur_password) ) && ($pmode < 2) )
				{
					$error = TRUE;
	...
	//
	if (( $email != $userdata['user_email'] ) && ($pmode < 2))
	{
	...
	if ( !$error )
	{
		if ( $avatar_sql == '' )
		{
			...
			if ($pmode == "2") {
			$sql = "UPDATE " . USERS_TABLE . " SET user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_aim = '" . str_replace("\'", "''", str_replace(' ', '+', $aim)) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_sig_bbcode_uid = '$signature_bbcode_uid' WHERE user_id = $user_id";
			}
			else if ($pmode == "3") {
			$sql = "UPDATE " . USERS_TABLE . " SET user_viewemail = $viewemail, user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '$user_actkey' WHERE user_id = $user_id";
			}
			else if (isset($HTTP_POST_VARS['submitavatar'])) {
			$sql = "UPDATE " . USERS_TABLE . " SET user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . " WHERE user_id = $user_id";
			}
			else if ($pmode == "4") {
			$sql = "UPDATE " . USERS_TABLE . " SET user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . " WHERE user_id = $user_id";
			}
			else {
			$sql = "UPDATE " . USERS_TABLE . " SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "' WHERE user_id = $user_id";
			}
...
else
{
	...
	$s_hidden_fields = '<input type="hidden" name="mode" value="' . $mode . '" /><input type="hidden" name="pmode" value="' . $pmode . '" /><input type="hidden" name="agreed" value="true" />';
	$s_hidden_fields .= '<input type="hidden" name="user_id" value="' . $userdata['user_id'] . '" />';
	//
	// Send the users current email address. If they change it, and account activation is turned on
	// the user account will be disabled and the user will have to reactivate their account.
	//
	$s_hidden_fields .= '<input type="hidden" name="current_email" value="' . $userdata['user_email'] . '" /><input type="hidden" name="username" value="' . $userdata['username'] . '" />';
	...
	if ($pmode == "2") {
		$template->set_filenames(array(
			'body' => 'profile_edit2.tpl')
		);
		$ploc = " -> <a href=\"".append_sid($phpbb_root_path . $profilelink.'pmode=2')."\" class=\"nav\">".$lang['Profile']."</a>";
	}
	else if ($pmode == "3") {
		$template->set_filenames(array(
			'body' => 'profile_edit3.tpl')
		);
		$ploc = " -> <a href=\"".append_sid($phpbb_root_path . $profilelink.'pmode=3')."\" class=\"nav\">".$lang['Forum']."</a>";
	}
	else if ($pmode == "4") {
		$template->set_filenames(array(
			'body' => 'profile_edit4.tpl')
		);
		$ploc = " -> <a href=\"".append_sid($phpbb_root_path . $profilelink.'pmode=4')."\" class=\"nav\">".$lang['Avatar']."</a>";
	}
	else {
		$template->set_filenames(array(
			'body' => 'profile_edit1.tpl')
		);
		$ploc = " -> <a href=\"".append_sid($phpbb_root_path . $profilelink.'pmode=1')."\" class=\"nav\">".$lang['User']."</a>";
	}
    ...
    $plocation = " -> <a href=\"".append_sid($phpbb_root_path . 'profile.php?mode=editprofile')."\" class=\"nav\">".$lang['Profile']."</a>".$ploc;
	...
	$template->assign_vars(array(
		...
		'S_PROFILE_LINK1'	=> "profile.php?mode=editprofile&pmode=1",
		'S_PROFILE_LINK2'	=> "profile.php?mode=editprofile&pmode=2",
		'S_PROFILE_LINK3'	=> "profile.php?mode=editprofile&pmode=3",
		'S_PROFILE_LINK4'	=> "profile.php?mode=editprofile&pmode=4",
